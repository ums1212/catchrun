...
#### 별도 컬렉션(gameCodes) 사용 이유
- **최적의 조회 성능 (O(1) Lookup)**: 9자리 번호 자체를 문서 ID로 사용함으로써, 대량의 게임 중에서도 쿼리(Query) 없이 단일 문서 가져오기(`get`)만으로 즉시 게임 정보를 찾아낼 수 있습니다.
- **번호 중복의 원천 차단 (Uniqueness)**: Firestore 문서 ID의 유일성 특성을 활용하여, 동일한 9자리 번호가 중복 생성되는 것을 데이터베이스 계층에서 보장합니다.
- **읽기 비용 및 보안 최적화**: 게임 전체 정보(`games` 문서)를 불러오기 전에 번호의 유효성을 최소한의 데이터로 먼저 확인하여 불필요한 리소스 낭비를 방지합니다.
